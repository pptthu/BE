...
def main():
    ...
    with app.app_context():
        ...
        if not User.query.filter_by(email="admin@example.com").first():
            u = User(email="admin@example.com", full_name="Admin", role_id=admin.id); u.set_password("Aa@123456"); db.session.add(u)
        if not User.query.filter_by(email="manager@example.com").first():
            u = User(email="manager@example.com", full_name="Manager", role_id=manager.id); u.set_password("Aa@123456"); db.session.add(u)

        loc1 = Location.query.filter_by(name="Campus A").first()
        if not User.query.filter_by(email="staff@example.com").first():
            u = User(email="staff@example.com", full_name="Staff A", role_id=staff.id, location_id=loc1.id if loc1 else None); u.set_password("Aa@123456"); db.session.add(u)
        if not User.query.filter_by(email="customer@example.com").first():
            u = User(email="customer@example.com", full_name="Customer", role_id=customer.id); u.set_password("Aa@123456"); db.session.add(u)
