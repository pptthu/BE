...
class BookingRepository:
    ...
    def summary(self, date_from: Optional[datetime] = None, date_to: Optional[datetime] = None) -> Dict[str, Any]:
        q = self.db.query(
        ...
        row = q.one()
        return {"bookings": int(row.bookings or 0), "revenue": float(row.revenue or 0.0)}
