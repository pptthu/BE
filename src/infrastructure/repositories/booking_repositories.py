...
class BookingRepository(BaseRepository[Booking]):
    ...
    def overlaps(self, pod_id: int, start: datetime, end: datetime) -> List[Booking]:
        # Có giao nhau nếu: existing.end > start && existing.start < end
        return (
            Booking.query
            .filter_by(pod_id=pod_id)
            .filter(Booking.end_time > start, Booking.start_time < end)
            .all()
        )
    ...
    def list_in_day(self, start_dt: datetime, end_dt: datetime) -> List[Booking]:
        return (
            Booking.query
            .filter(Booking.start_time >= start_dt, Booking.start_time <= end_dt)
            .all()
        )
